	...
	getWrite(function(err, db){
		...
		db.collection('ids', function(err, collectionIds){
			...
			async.parallel([
				...
				function(callback){		// 消息表
					collectionIds.findOne({table:'message'}, function(err, item){
						if(err){ return callback(err); }
						if(!item){
							collectionIds.insert({table:'message', id:0}, function(err, result){
								if(err){ return callback(err); }
								return callback();
							});
						}else{ return callback(); }
				...
				function(callback){		// 收藏表
					collectionIds.findOne({table:'collect'}, function(err, item){
						if(err){ return callback(err); }
						if(!item){
							collectionIds.insert({table:'collect', id:0}, function(err, result){
								if(err){ return callback(err); }
								return callback();
							});
						}else{ return callback(); }
				...
				function(callback){		// 收藏表
					collectionIds.findOne({table:'follow'}, function(err, item){
						if(err){ return callback(err); }
						if(!item){
							collectionIds.insert({table:'follow', id:0}, function(err, result){
								if(err){ return callback(err); }
								return callback();
							});
						}else{ return callback(); }
				...
				function(callback){		// 收藏表
					collectionIds.findOne({table:'share'}, function(err, item){
						if(err){ return callback(err); }
						if(!item){
							collectionIds.insert({table:'share', id:0}, function(err, result){
								if(err){ return callback(err); }
								return callback();
							});
						}else{ return callback(); }
				...
				function(callback){		// 收藏表
					collectionIds.findOne({table:'comment'}, function(err, item){
						if(err){ return callback(err); }
						if(!item){
							collectionIds.insert({table:'comment', id:0}, function(err, result){
								if(err){ return callback(err); }
								return callback();
							});
						}else{ return callback(); }
